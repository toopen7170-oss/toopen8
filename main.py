...
# [진단] 갤럭시 S26 울트라 최적화: 자판 대응 및 화면 고정
from kivy.config import Config
Config.set('kivy', 'keyboard_mode', 'system_and_dock')
Config.set('kivy', 'softinput_mode', 'pan') # 자동 스크롤: 자판 위로 입력창 이동
...
class PristonTaleApp(MDApp):
    ...
    def run_diagnosis(self, dt):
        ...
        msg = "\n".join(errs) if errs else "✅ 모든 시스템 무결성 검사 완료 (오류 0개)"
        d = MDDialog(title="🛡️ 라인 바이 라인 전수 진단", text=msg, buttons=[MDRaisedButton(text="확인", on_release=lambda x: d.dismiss())])
        d.open()
    ...
    def confirm(self, title, text, on_yes):
        ...
        d = MDDialog(title=title, text=text, buttons=[
            ...
            MDRaisedButton(text="확인", md_bg_color=(1,0,0,1) if "삭제" in title else (0,0.5,0,1), 
                           on_release=lambda x: [on_yes(), d.dismiss()])
        ])
        d.open()
...
class AccountScreen(Screen):
    ...
    def setup_ui(self):
        ...
        self.new_id = MDTextField(hint_text="새 계정 ID 입력", halign="center")
        l.add_widget(self.new_id)

        l.add_widget(MDRaisedButton(text="계정 저장", md_bg_color=(0,0.5,0,1), size_hint_x=1, 
                                     on_release=lambda x: MDApp.get_running_app().confirm("저장", "저장하시겠습니까?", self.save)))
    ...
    def refresh(self, q=""):
        ...
        for (n,) in cur.fetchall():
            ...
            d = MDRaisedButton(text="삭제", md_bg_color=(1,0,0,1), size_hint_x=0.2); 
            d.bind(on_release=lambda x, name=n: MDApp.get_running_app().confirm("삭제", f"[{name}] 삭제하시겠습니까?", lambda: self.delete(name)))
...
class CharSelectScreen(Screen):
    def on_enter(self):
        self.clear_widgets()
        p = os.path.join(BASE_DIR, "bg.png")
        if os.path.exists(p): self.add_widget(Image(source=p, opacity=0.15, allow_stretch=True, keep_ratio=False))
        ...
        for i in range(1, 7):
            ...
            grid.add_widget(btn)
        l.add_widget(grid)
        l.add_widget(MDRectangleFlatButton(text="계정 선택으로", size_hint_x=1, on_release=lambda x: setattr(self.manager, 'current', 'acc')))
        self.add_widget(l)
...
class SlotMenuScreen(Screen):
    def on_enter(self):
        ...
        menu_items = [("케릭정보창", "info"), ("케릭장비창", "equ"), ("인벤토리창", "inv"), ("사진선택창", "pho")]
        for n, t in menu_items:
            btn = MDRaisedButton(text=n, size_hint_x=1, height=dp(65), font_style="H6", md_bg_color=(0,0.4,0.6,1))
...
class CharInfoScreen(Screen):
    def __init__(self, **kw):
        ...
        sv = MDScrollView(); self.cont = BoxLayout(orientation='vertical', padding=dp(15), spacing=dp(20), size_hint_y=None)
        self.cont.bind(minimum_height=self.cont.setter('height'))
        
        for g in self.groups:
            gb = BoxLayout(orientation='vertical', spacing=dp(5), size_hint_y=None)
            ...
            self.cont.add_widget(gb) 

        self.cont.add_widget(MDRaisedButton(text="정보 저장", md_bg_color=(0,0.5,0,1), size_hint_x=1, on_release=lambda x: self.save()))
        self.cont.add_widget(MDRaisedButton(text="전체 삭제", md_bg_color=(1,0,0,1), size_hint_x=1, 
                                            on_release=lambda x: MDApp.get_running_app().confirm("삭제", "정보를 초기화하시겠습니까?", self.clear)))
...
class EquipScreen(Screen):
    def __init__(self, **kw):
        ...
        l.add_widget(MDRaisedButton(text="전체 삭제", md_bg_color=(1,0,0,1), size_hint_x=1, 
                                     on_release=lambda x: MDApp.get_running_app().confirm("삭제", "장비 목록을 비우시겠습니까?", self.clear)))
...
class InvenScreen(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.inputs = {}
        sv = MDScrollView(); l = BoxLayout(orientation='vertical', padding=dp(20), spacing=dp(10), size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        
        for i in range(1, 21):
            tf = MDTextField(hint_text=f"인벤토리 {i:02d}번 줄", mode="rectangle", halign="center")
            self.inputs[i] = tf; l.add_widget(tf)
            
        l.add_widget(MDRaisedButton(text="인벤토리 저장", md_bg_color=(0,0.5,0,1), size_hint_x=1, on_release=lambda x: self.save()))
        l.add_widget(MDRectangleFlatButton(text="뒤로", size_hint_x=1, on_release=lambda x: setattr(self.manager, 'current', 'slot_menu')))
        sv.add_widget(l); self.add_widget(sv)

    def on_pre_enter(self):
        app = MDApp.get_running_app(); cur.execute("SELECT data FROM chars WHERE acc=? AND slot=? AND type='inv'", (app.acc, app.slot))
        res = cur.fetchone(); data = json.loads(res[0]) if res else {}
        for k, tf in self.inputs.items(): tf.text = data.get(str(k), "")
    def save(self):
...
class PhotoScreen(Screen):
    def __init__(self, **kw):
        ...
        self.img = Image(source="", size_hint_y=0.7, allow_stretch=True)
        self.l.add_widget(self.img)
        
        self.l.add_widget(MDRaisedButton(text="핸드폰 사진 업로드", md_bg_color=(0,0.4,0.7,1), size_hint_x=1, on_release=self.pick))
    ...
    def on_selection(self, selection):
        if selection:
            ...
            MDApp.get_running_app().confirm("저장", "이 사진을 등록하시겠습니까?", lambda: print("Photo Saved"))
