...
class PristonTaleApp(App):
    def build(self):
        ...
        for eq in equip_list:
            eq_box = BoxLayout(size_hint_y=None, height=40, spacing=5)
            eq_box.add_widget(Label(text=eq, font_name=f_name))
            eq_box.add_widget(TextInput(multiline=False, font_name=f_name))
            content.add_widget(eq_box)

        # [5] 인벤토리 / [6] 사진 관리 (생략 없이 구조 유지)
        ...
        for i in range(3):
            ...
            inv_line.add_widget(Button(text=f"아이템 {i+1}", font_name=f_name))
