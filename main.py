...
class PristonTaleApp(App):
    def build(self):
        ...
        for group in info_groups:
            ...
            content.add_widget(BoxLayout(size_hint_y=None, height=15))
        ...
        # [5] 인벤토리 및 사진 관리
        content.add_widget(Label(text="[ 데이터 관리 ]", size_hint_y=None, height=40, font_name=f_name))
        btn_box = BoxLayout(size_hint_y=None, height=50, spacing=5)
        btn_box.add_widget(Button(text="사진 선택", on_release=self.open_file_chooser, font_name=f_name))
        btn_box.add_widget(Button(text="저장", font_name=f_name))
    ...
    def open_file_chooser(self, instance):
        ...
        btn = Button(text="닫기", size_hint_y=None, height=50, on_release=lambda x: popup.dismiss())
        content.add_widget(btn)
        popup = Popup(title="파일 선택", content=content, size_hint=(0.9, 0.9))
        popup.open()
