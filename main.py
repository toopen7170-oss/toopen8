...
# [전수검사 반영] 안드로이드 앱 내부 리소스 경로 강제 지정
...
# --- 제1 기본원칙: 목록과 선택창 구조 절대 보존 영역 ---

class MainMenu(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        add_bg(self, 1.0) # 메인 화면 배경
        ...
        for name, screen_name in menus:
            btn = MDRaisedButton(
                text=name, size_hint=(1, None), height=dp(55),
...
class CharInfoScreen(Screen):
    def __init__(self, **kwargs):
        ...
        for group in groups:
            ...
            layout.add_widget(BoxLayout(size_hint_y=None, height=dp(20))) # 화면에 안보이는 여백
